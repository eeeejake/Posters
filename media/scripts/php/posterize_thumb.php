  ...
  if (is_uploaded_file($_FILES['image']['tmp_name'])) {
    ...
    $name = preg_replace($imagetypes, '', basename($_FILES['image']['name']));
     ...
     // process image with gd library to posterize it
	  $poster = imagecreatefromjpeg($original);//file  which was moved to upload dir, must by jpeg
                set_time_limit(0);
                ...
                //imagefilter($poster, IMG_FILTER_EDGEDETECT);
                //imagefilter($poster, IMG_FILTER_NEGATE);
                //imagefilter($poster, IMG_FILTER_GRAYSCALE);
                //imagefilter($poster, IMG_FILTER_SMOOTH, 20);
